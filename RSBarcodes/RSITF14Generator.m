...
- (NSString *)barcode:(NSString *)contents
{
    ...
    for (int i = 0; i < contents.length / 2; i++) {
        ...
        NSString *bars = ITF14_CHARACTER_ENCODINGS[[[pair substringWithRange:NSMakeRange(0, 1)] intValue]];
        NSString *spaces = ITF14_CHARACTER_ENCODINGS[[[pair substringWithRange:NSMakeRange(1, 1)] intValue]];
        for (int j = 0; j < 10; j++) {
            if (j % 2 == 0) {
